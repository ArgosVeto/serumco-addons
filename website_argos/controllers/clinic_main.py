...
class MailingList(http.Controller):
    ...
    def update_mailinglist(self, **kwargs):
        ...
        if 'news' in kwargs and kwargs['news']:
            mailing_list_id = request.env['mailing.list'].sudo().browse(
                int(kwargs['news']))
        ...
        if email and mailing_list_id:
            ...
            ContactSubscription.create(
                {'contact_id': contact_id.id, 'list_id': int(mailing_list_id.id)})
    ...
    def application_confirm(self, **kwargs):
        ...
        if 'agglomeration_id' in kwargs and kwargs['agglomeration_id']:
            applicant_vals.update(
                {'agglomeration_id': kwargs['agglomeration_id']})
        ...
        if 'div_file1' in kwargs and kwargs['div_file1']:
            ir_attachment_id = request.env['ir.attachment'].sudo().browse(
                int(kwargs['div_file1']))
            if ir_attachment_id and app_id:
                ir_attachment_id.res_id = app_id.id
                ir_attachment_id.res_model = 'hr.applicant'
        if 'div_file2' in kwargs and kwargs['div_file2']:
            ir_attachment_id = request.env['ir.attachment'].sudo().browse(
                int(kwargs['div_file2']))
            if ir_attachment_id and app_id:
                ir_attachment_id.res_id = app_id.id
                ir_attachment_id.res_model = 'hr.applicant'
...
class WebsiteHrRecruitment(WebsiteHrRecruitment):
    ...
    def jobs(self, country=None, department=None, office_id=None, **kwargs):
        ...
        if 'search_job_type' in kwargs and kwargs['search_job_type']:
            job_type_id = request.env['job.type'].sudo().search(
                [('id', '=', int(kwargs['search_job_type']))])
        ...
        if 'search' in kwargs and kwargs['search']:
            ...
            subdomains.append(
                [('address_id.street', 'ilike', kwargs['search'])])
            subdomains.append(
                [('address_id.street2', 'ilike', kwargs['search'])])
...
class WebsiteForm(WebsiteForm):
    ...
    def website_form(self, model_name, **kwargs):
        ...
        if model_name == 'hr.applicant' and request.session['form_builder_id']:
            applicant_vals = {}
            partner_name = ''
            hr_applicant_id = request.env['hr.applicant'].sudo().browse(
            ...
            if 'number_of_exp' in kwargs and kwargs['number_of_exp']:
                applicant_vals.update(
                    {'number_of_exp': kwargs['number_of_exp']})
            if 'address' in kwargs and kwargs['address']:
                applicant_vals.update({'address': kwargs['address']})
            if 'code_postal' in kwargs and kwargs['code_postal']:
                applicant_vals.update({'code_postal': kwargs['code_postal']})
            ...
            if 'agglomeration_id' in kwargs and kwargs['agglomeration_id']:
                applicant_vals.update(
                    {'agglomeration_id': int(kwargs['agglomeration_id'])})
            if 'job_type_id' in kwargs and kwargs['job_type_id']:
                applicant_vals.update(
                    {'job_type_id': int(kwargs['job_type_id'])})
            applicant_vals.update(
                {'partner_name': partner_name, 'description': ''})
...
class WebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if 'ptal' in kw and kw['ptal']:
            ...
            if product_tmpl_id:
                cart_product_id = request.env['product.template'].sudo().browse(
                    product_tmpl_id)
                if cart_product_id.product_variant_ids:
                    product_variant_ids = cart_product_id.product_variant_ids.filtered(
                        lambda s:  int(kw['ptal']) in s.product_template_attribute_value_ids.ids)
...
class ClinicDetail(http.Controller):
    ...
    @http.route(['/update-fav-delivery-address'], type='json', auth="public", website=True)
    ...
    @http.route(['/bouton-paiement'], type='json', auth="public", website=True)
    def bouton_paiement(self, **post):
        partner = request.env.user.partner_id
        sale_order_id = request.session['sale_order_id']
        sale_order = request.env['sale.order'].sudo().browse(
            sale_order_id).exists() if sale_order_id else None
        ...
        values = {'website_sale_order': sale_order,
                  'clinic_fav': fav_clinic,
                  'click_and_collect': click_and_collect}
        return request.env['ir.ui.view'].render_template("website_argos.bouton_paiement", values)
    ...
    def clinic_detail(self, **post):
        subdomains = []
        domain = [[('active', '=', True)]]
        clinic_collect_ids = request.env['operating.unit'].sudo().search(
            [('click_and_collect', '=', True)])
        clinic_appoint_ids = request.env['operating.unit'].sudo().search(
            [('online_appointment_booking', '=', True)])

        if 'clinic-collect' in post and post['clinic-collect']:
            clinic_type_ids = request.env['operating.unit'].sudo().search(
                [('click_and_collect', '=', True)])
        ...
        if 'open_today' in post and post['open_today'] == str(1):
            ...
            for unit in operating_unit_ids:
                if unit.calendar_id and unit.calendar_id.attendance_ids:
                    day_timimg = unit.calendar_id.attendance_ids.filtered(
                        lambda x: x.dayofweek == str(date_today))
                    if day_timimg:
    ...
    def add_clinic_favorite(self, operating_unit, **post):
        ...
        if partner_id.clinic_shortlisted_ids.ids:
            check_operating_uni = any(
                operating_unit.id == cs.favorite_clinic_id.id for cs in partner_id.clinic_shortlisted_ids)
...
class ClinicContact(http.Controller):
    ...
    def clinic_contact(self, **post):
        ...
        values = {'clinic_ids': clinic_ids, 'partner_ids': partner_ids,
                  'contact_questions_ids': contact_questions_ids}
