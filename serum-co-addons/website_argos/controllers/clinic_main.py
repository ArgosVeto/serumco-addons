...
class ClinicDetail(http.Controller):
    ...
    def update_delivery_address(self,**post):
        ...
        if partner.clinic_shortlisted_ids:
            fav_clinic = partner.clinic_shortlisted_ids[0]
    ...
    def clinic_detail(self,**post):
        ...
        if 'parking' in post and post['parking'] == 'True':
            operating_ids = request.env['operating.unit'].sudo().search([('parking', '=', True)])
    ...
    def add_clinic_pratice(self,operating_unit,**post): 
        ...
        values = {'service_ids':service_ids,'operating_unit':operating_unit_id,'working_timing':working_timing,'not_working_time':not_working_time}
