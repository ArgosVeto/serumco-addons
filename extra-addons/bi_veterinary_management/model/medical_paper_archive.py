...
class medical_paper_archive(models.Model):
    ...
    location_id = fields.Many2one('medical.hospital.unit','Unit', required=True)
    hc_status = fields.Selection([
            ('archived', 'Ambulatory'),
            ('borrowed', 'Borrowed'),
            ('lost', 'Lost'),
        ], 'Status', sort=False, required=True)
    current_location_id = fields.Many2one('medical.hospital.unit','Current Location', required=True)
    partner_id = fields.Many2one('res.partner','Requested by', required=True)
