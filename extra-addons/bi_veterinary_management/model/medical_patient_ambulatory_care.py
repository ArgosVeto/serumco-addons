...
class medical_patient_ambulatory_care(models.Model):
    ...
    pathology_base_condition_id = fields.Many2one('medical.pathology',string='Base Condition')
    ...
    patient_evaluation_id = fields.Many2one('medical.patient.evaluation',string='Related Evaluation')
    session_start = fields.Datetime(string='Start',required=True)
    state = fields.Selection([('done','Done'),
                              ('draft','Draft')],
                             string='Status')
    procedures_ids = fields.One2many('medical.ambulatory_care_procedure','medical_patient_ambulatory_care_procedure_id',string="Procedures")
    ...
    moves_ids = fields.One2many('stock.move','medical_patient_rounding_move_id',string="Moves")