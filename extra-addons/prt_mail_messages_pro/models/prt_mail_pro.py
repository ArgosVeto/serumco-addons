...
class PRTMailMessage(models.Model):
    ...
    def unlink(self):
        ...
        # Delete empty leads
        leads = self.env['crm.lead'].browse(lead_ids).filtered(
            lambda l: l.company_id.lead_delete and l.type == 'lead')
    ...
    def _get_message_format_fields(self):
        res = super(PRTMailMessage, self)._get_message_format_fields()
...
class PRTMailMove(models.TransientModel):
    ...
    def message_move(self):
        ...
        if self.is_conversation:
            messages = self.env['mail.message'].search([('model', '=', 'cetmix.conversation'),
        ...
        for message in messages:
            old_records.append([message.id, "%s_%s" % (message.model, str(message.res_id)),
                                "%s_%s" % (dest_model, str(dest_res_id)),
                                [message.model, message.res_id], [dest_model, dest_res_id]])

        # Store leads from messages in case we want to delete empty leads later
        leads = False
        if self.is_lead and self.lead_delete:
        ...
        conversations = False
        conversation_messages = messages.filtered(lambda m: m.model == 'cetmix.conversation')
        if len(conversation_messages) > 0:
            conversations = self.env['cetmix.conversation'].search([('id', 'in', conversation_messages.mapped('res_id'))])
        ...
        # Update notifications
        notifications = []
        partner_ids = [partner.id for partner in messages.mapped('ref_partner_ids')]
        if self.env.user.partner_id.id not in partner_ids:
            partner_ids.append(self.env.user.partner_id.id)
        for partner_id in partner_ids:
            notifications.append([
