odoo.define('argos_website.paymentForm', function (require) {
'use strict';
                ...
                if ($tx_url.length === 1) {
                    ...
                    return this._rpc({
                        ...
                        if (jQuery.type(result) == 'array') {
                            ...
                            newForm.setAttribute("action", "/shop/redirect/cart");
                            newForm.hidden = true; // hide it
                            var formbuild = "";
                            for (var i=0; i<result.length; i++) {
                                formbuild = formbuild + "<input type='hidden' name='unavailable_product_" + i.toString() + "' value='" +
                                result[i] +
                                "'/>"; //
                            }
                            formbuild = formbuild + "<input type='hidden' name='unavailable_products_length' value='" + result.length
                            .toString() + "'/>"
                            newForm.innerHTML = formbuild
                            ...
                            var newForm = document.createElement('form');
                            newForm.setAttribute("method", "post");
                            newForm.setAttribute("action", "/shop/error_service");
                            newForm.hidden = true; // hide it
                            $(document.getElementsByTagName('body')[0]).append(newForm);
                            newForm.submit();
                            return new Promise(function () {});
